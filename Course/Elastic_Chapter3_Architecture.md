...
اول برای فیلدهای متنی (`text`) مثل `name` و `city`، الستیک‌سرچ از **Analyzer** استفاده می‌کند تا متن را به توکن‌های (Tokens) کوچک‌تر تجزیه کند.
...
پس از ایندکسینگ، سند در الستیک‌سرچ به این شکل ذخیره می‌شود:
 ...
 یا به صورت **داده خام (Raw Data)**: سند JSON اصلی در فیلد `_source` ذخیره می‌شود:
   ```json
   {
     "_index": "my_index",
     "_id": "1",
 ...
 یا به صورت **Inverted Index**: برای جستجو، ساختار زیر (به صورت مفهومی) ایجاد می‌شود:
 ...
 یا به صورت **Metadata**: اطلاعاتی مثل `_index`، `_id`، و نسخه سند (`_version`) برای مدیریت و ردیابی تغییرات ذخیره می‌شود.
