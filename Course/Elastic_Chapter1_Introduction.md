...
#### مشکل شای بانون چی بود؟

در سال ۲۰۱۰، Shay Banon در حال ساخت یک اپلیکیشن آشپزی برای همسرش بود ;i با یک مشکل مشخص روبه‌رو شد: **ساختن یک سیستم جستجوی ساده و کارآمد برای حجم زیادی از داده‌ها سخت بود.**
او می‌خواست برای همسرش که در حال تحصیل برای سرآشپز شدن بود، یک ابزار جستجو درست کنه تا بتونه توی مجموعه‌ی بزرگ دستور پخت‌ها (recipes) به راحتی جستجو کنه.
 > مثلاً بگه کدوم دستورها "مرغ" دارن یا کدوم‌ها سریع آماده می‌شن.

در اون زمان، شای از **Apache Lucene** استفاده می‌کرد، که یه کتابخونه‌ی متن‌باز و قدرتمند برای جستجو بود. 

⚠️ اما Lucene یه مشکل بزرگ داشت:
-  **سطح پایین (low-level) بود**
-  **استفاده ازش پیچیده و زمان‌بر بود.**

 شای دنبال سیستمی بود که هم قدرتمند باشه، هم مقیاس‌پذیر، و هم استفاده‌ش برای توسعه‌دهنده‌ها راحت باشه.
...
### چه ربطی به نرم‌افزار آشپزی داشت؟

ارتباطش خیلی مستقیم بود! همسر شای بانون یه مجموعه‌ی بزرگ از دستور پخت‌ها داشت و شای می‌خواست یه نرم‌افزار جستجوی شخصی بسازد تا این دستورها رو سازمان‌دهی و قابل جستجو کنه. مثلاً اگه همسرش می‌خواست دستور پخت‌هایی با "ماهی" پیدا کنه که کمتر از ۴۰۰ کالری داشته باشن، این نرم‌افزار باید سریع جواب می‌داد. این نیاز شخصی، جرقه‌ی اولیه‌ی ساخت Elasticsearch رو زد، چون شای دید که ابزارهای موجود مثل Lucene برای این کار بهینه نیستن.

---

### Apache Lucene چه سختی‌ای داشت؟

Lucene یه کتابخونه‌ی قوی برای جستجو بود، اما چون سطح پایین بود، توسعه‌دهنده رو مجبور می‌کرد خیلی از کارها رو دستی انجام بده. سختی‌هاش اینا بودن:

- **مدیریت ایندکسینگ**: باید خودتون داده‌ها رو تبدیل به ایندکس می‌کردید، مثلاً متن رو به فهرست معکوس (inverted index) تبدیل کنید.
- **توزیع‌شدگی**: اگه داده‌ها زیاد باشن و روی یه سرور جا نشن، باید خودتون سیستم رو بین چند سرور پخش کنید، که یعنی کد نویسی شبکه و هماهنگی سرورها.
...
فرض کنید یه وب‌سایت خبری با ۵ میلیون مقاله دارید و می‌خواهید کاربران بتونن توی مقالات جستجو کنن. با Lucene:

1. باید کد بنویسید که هر مقاله رو بخونه و ایندکس کنه (مثلاً کلمات کلیدی رو استخراج کنه).
...
این کارها برای یه پروژه‌ی ساده مثل دستور پخت‌ها هم خیلی زیاد بود و شای رو کلافه کرده بود.

---

### راهکار شای بانون چی بود؟

شای بانون تصمیم گرفت یه لایه‌ی جدید روی Lucene بسازد که این مشکلات رو حل کنه. راهکارش **Elasticsearch** بود، یه موتور جستجو و تحلیل توزیع‌شده که استفاده ازش ساده باشه و قدرت Lucene رو حفظ کنه.
...
1. مقالات رو به صورت JSON می‌فرستید:
   ```json
   {
     ...
     "content": "تیم ملی برنده شد...",
     "date": "2023-10-01"
   }
   ```
2. Elasticsearch خودش داده‌ها رو ایندکس می‌کنه و توی شارد‌ها پخش می‌کنه.
3. برای جستجو، یه درخواست ساده می‌فرستید:
   ```
   GET /news/_search?q=ورزشی
   ```
...
سیستم Elasticsearch بر پایه‌ی **Apache Lucene** ساخته شده؛ یک موتور قدرتمند برای ایندکس‌گذاری و جستجو، که توسط خود آپاچی توسعه داده شد اما کار باهاش پیچیده بود.
ابزار Elasticsearch اومد و رابط RESTful، مقیاس‌پذیری و سادگی رو به اون اضافه کرد.


یه برنامه‌نویس به اسم **Shay Banon** تصمیم گرفت ابزاری بسازد که جستجو و تحلیل داده‌های بزرگ رو راحت‌تر کنه.
این ابزار، که اسمش **Elasticsearch** بود، بر پایه‌ی یه موتور جستجوی متن‌باز به اسم **Apache Lucene** ساخته شد.
آپاچی Lucene خودش خیلی قوی بود، ولی برای کار با داده‌های عظیم و تیمی به یه لایه‌ی بالاتر نیاز داشت. 
