...
### نسل قبل دیتابیس 👨‍🦳 ها vs مقابل الستیک 👶
...
#### مثال  
 شما صدها سرور، اپلیکیشن و سرویس دارید. هر کدام در حال لاگ‌سازی هستند.
...
#### مثال  
یه دفترچه یادداشت دارید که توش هر خط یه لاگ از وب‌سایتتونه. حالا می‌خواهید همه‌ی خط‌هایی که کلمه‌ی "error" توشون هست رو پیدا کنید:
...
توی این روش، مانیتورینگ (نظارت) و عیب‌یابی سریع خیلی مهمه.Elastic اینجا وارد می‌شه:
...
این یعنی درد "کندی و سختی جستجو توی داده‌های بزرگ" رو حل کرده.
...
### بخش Logstash چیه و چرا ازش استفاده می‌کنیم؟

این Logstash یه ابزاره توی استک Elasticه که کارش جمع‌آوری، پردازش و ارسال داده‌هاست.
...
- **مثل یه پستچی باهوشه که:**

  1. **جمع‌آوری**: لاگ‌ها رو از فایل‌ها، سرورها یا هر جای دیگه می‌گیره.
    
  2. **پردازش**: داده‌ها رو مرتب می‌کنه (مثلاً تاریخ و ساعت رو جدا می‌کنه).
    
  3. **ارسال**: به Elasticsearch می‌فرسته تا ذخیره بشن.
    
> چون خود لاگ های خام برای الستیک هم مشکل ساز هستند.
